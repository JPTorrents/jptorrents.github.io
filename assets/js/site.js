(function () {
  ...
  function initScrollSpy() {
    ...
    var observer = new IntersectionObserver(function (entries) {
      ...
      var active = Array.from(visible.entries()).sort(function (a, b) { return b[1] - a[1]; })[0][0];
      activateToc(active);
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    initScrollSpy();
    document.querySelectorAll('.toc a[href^="#"]').forEach(function (link) {
      link.addEventListener('click', function () { activateToc(link.getAttribute('href').slice(1)); });
    });
  });
}());
